...
Yolov8Face::Yolov8Face(string model_path, const float conf_thres, const float iou_thresh)
{
	...
	//this->input_names.push_back(input_name_Ptr.get()); /// 高版本onnxruntime的接口函数
	this->input_names.push_back(mem); /// 高版本onnxruntime的接口函数
	...
	memcpy(mem, output_name_Ptr.get(), strlen(output_name_Ptr.get()) + 1);
...
void Yolov8Face::preprocess(Mat srcimg) {
    ...
    if (height > this->input_height || width > this->input_width) {
        const float scale = std::min((float)this->input_height / height,
                (float)this->input_width / width);
        Size new_size = Size(int(width * scale), int(height * scale));
        resize(srcimg, temp_image, new_size);
    }

    printf("%s (%d) tmp height = %d, width = %d\n",
            __func__, __LINE__, temp_image.rows, temp_image.cols);

    this->ratio_height = (float) height / temp_image.rows;
    this->ratio_width = (float) width / temp_image.cols;
    printf("%s (%d) ratio_height = %f, ratio_width = %f\n",
            __func__, __LINE__, this->ratio_height, this->ratio_width);

    printf("%s (%d) memory %lld\n", __func__, __LINE__,
            3 * this->input_height * this->input_width * sizeof(float));

    printf("%s (%d) memory %lld\n", __func__, __LINE__,
            this->input_height * this->input_width * sizeof(float));
    ...
    float *d_input_image, *d_bgrChannels_0, *d_bgrChannels_1, *d_bgrChannels_2;
    cudaMalloc((void **)&d_input_image, this->input_height * this->input_width * sizeof(float));
    cudaMalloc((void **)&d_bgrChannels_0, this->input_height * this->input_width * sizeof(float));
    cudaMalloc((void **)&d_bgrChannels_1, this->input_height * this->input_width * sizeof(float));
    cudaMalloc((void **)&d_bgrChannels_2, this->input_height * this->input_width * sizeof(float));

    //copy data from CPU to GPU
    cudaMemcpy(d_bgrChannels_0, temp_image.data, this->input_height *
            this->input_width * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_bgrChannels_1, temp_image.data + height * width, this->input_height *
            this->input_width * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_bgrChannels_2, temp_image.data + 2 * height * width, this->input_height *
            this->input_width * sizeof(float), cudaMemcpyHostToDevice);

#if 0
    //define block and grid dimensions
    dim3 blockSize(16, 16);
    dim3 gridSize((this->input_width + blockSize.x - 1) / blockSize.x, (this->input_height + blockSize.y - 1) / blockSize.y);

    //call the cuda kernel
    preprocessKernel<<<gridSize, blockSize>>>(d_input_image, d_bgrChannels_0, d_bgrChannels_1,
            d_bgrChannels_2, this->input_height, this->input_width);
    cudaDeviceSynchronize();
#endif

#if 1
    printf("enter\n");
    preprocessKernelFloat(d_input_image, d_bgrChannels_0, d_bgrChannels_1,
            d_bgrChannels_2, this->input_height, this->input_width);
#endif

    cudaMemcpy(this->input_image.data(), d_input_image, 3 * this->input_height * this->input_width
            * sizeof(float), cudaMemcpyDeviceToHost);

    cudaFree(d_input_image);
    cudaFree(d_bgrChannels_0);
    cudaFree(d_bgrChannels_1);
    cudaFree(d_bgrChannels_2);
}
#endif
...
void Yolov8Face::detect(Mat srcimg, std::vector<Bbox> &boxes)
{
    ...
    //Value input_tensor_ = Value::CreateTensor<float>(memory_info_handler, this->input_image.data(), /*this->input_image.size()*/this->input_height * this->input_width, input_img_shape.data(), input_img_shape.size());
    Value input_tensor_ = Value::CreateTensor<float>(memory_info_handler, this->input_image.data(), /*this->input_image.size()*/4915200, input_img_shape.data(), input_img_shape.size());
    ...
    vector<Value> ort_outputs = this->ort_session->Run(runOptions, this->input_names.data(), &input_tensor_, 1, this->output_names.data(), 1);
